...
const initializeOrderShipmentsTable = async () => {
  try {
    ...
    // Check if production_planning table exists and has data
    const ppExists = await sql`SELECT to_regclass('public.production_planning') AS reg`;
    if (ppExists.length && ppExists[0].reg) {
      // Check if there are any order_shipments records that don't have corresponding production_planning records
      const orphanedRecords = await sql`
        SELECT DISTINCT os.order_id 
        FROM order_shipments os 
        LEFT JOIN production_planning pp ON os.order_id = pp.order_id 
        WHERE pp.order_id IS NULL
      `;
      
      if (orphanedRecords.length > 0) {
        console.log(`⚠️ Found ${orphanedRecords.length} orphaned order_shipments records, cleaning up...`);
        // Delete orphaned records to prevent FK constraint violation
        await sql`DELETE FROM order_shipments WHERE order_id = ANY(${orphanedRecords.map(r => r.order_id)})`;
        console.log('✅ Cleaned up orphaned order_shipments records');
      }
...
const createInventoryItem = async (itemData) => {
  try {
    ...
    const duplicateCheck = await sql`
      ...
      WHERE (item_code = ${item_code} AND item_code IS NOT NULL)
         OR (product_id = ${product_id} AND product_id IS NOT NULL)
      LIMIT 1
    `;
    
    if (duplicateCheck.length > 0) {
      const error = new Error('Duplicate item detected');
...
const createOrderShipment = async (orderData) => {
  try {
    ...
    if (duplicateCheck.length > 0) {
      const error = new Error('Duplicate order shipment detected');
