...
const createNotification = async (title, message, type = 'info') => {
  try {
    ...
    if (!sql) {
      ...
      const isDuplicate = inMemoryNotifications.some(n => 
        n.title === title && 
        n.message === message && 
        n.type === type &&
        (new Date() - new Date(n.created_at)) < 60000 // Within last minute
      );
    ...
    // Check for duplicates in database (within last minute)
    ...
    try {
      ...
      if (!tableCheck[0]?.exists) {
        ...
        if (idColumnCheck.length > 0 && !idColumnCheck[0].column_default) {
          console.log('⚠️ ID column is not SERIAL, fixing...');
          ...
          `;
          console.log('✅ Notifications table recreated with proper SERIAL ID');
        }
      }
      ...
      // Try to create a simple notifications table as a last resort
      try {
        console.log('🔄 Attempting to create simple notifications table...');
        await sql`
          CREATE TABLE IF NOT EXISTS notifications (
        ...
        console.log('✅ Simple notifications table created');
        ...
        console.error('Simple table creation also failed:', simpleTableError);
